...
$sql_vendor="";
if(($hierarchy)&&(!is_siteadmin($USER->id))&&(!$is_vendor)){
	...
	if($is_vendor){
		// Add condition for the vendor
		$arr_course_ids = get_vendor_course_ids($USER->id);
	    $list = implode(",",$arr_course_ids);
	    $sql_vendor = " AND id in (SELECT ue.userid from mdl_user_enrolments as ue INNER JOIN mdl_enrol e 
	    ON ue.enrolid=e.id and e.courseid in($list) GROUP BY ue.userid) ";
	}
}
//$hierarchyquery="";
$find_all_users_sql = "SELECT DISTINCT concat(firstname, ' ', lastname) user_name
		FROM mdl_user 
		WHERE concat(firstname, ' ', lastname) LIKE '%" . addslashes($term) . "%' 
		".$hierarchyquery.$sql_vendor.
		" AND deleted <> 1
		AND suspended <> 1
		ORDER BY firstname";
