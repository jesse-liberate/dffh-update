...
$PAGE->set_url($CFG->wwwroot. '/blocks/coach/graduate_add.php');
...
if(isset($_POST['sub'])){
	...
	if(!empty($old)){
		foreach ($old as $row) {
			if(!in_array($row->coacheeid, $update)){
				...
				$DB->delete_records('coachees',array('coachid'=>$coachid,'coacheeid'=>$row->coacheeid,'is_student'=>GRADUTE_STUDENT));
				//NEED TO UPDATE AND REMOVE PROFILE FIELD
				$msg.= update_profile_field_value('IsGraduateStudent',$row->coacheeid,MDL_CHECKBOX_UNCHECK);
				// remove_user_position_profile($row->coacheeid,POSITION_STUDENT);
			}else{
	...
	if(!empty($update)){
		foreach ($update as $key=>$val) {
			...
			$new->coacheeid = $val;
			$new->coachid = $coachid;
			$new->is_student = GRADUTE_STUDENT;
			$new->timemodified = time();
			$DB->insert_record('coachees',$new);
			// update_user_position_profile($val,POSITION_STUDENT);
			$msg.= update_profile_field_value('IsGraduateStudent',$val,MDL_CHECKBOX_CHECK);
		}
	}
	if($msg==""){ 
...
if(isset($_GET['id'])){
	...
	$graduate_student_options=get_available_students_options($coachid,true);
	...
	$html.="<form action='graduate_add.php' method='POST'>";
	...
	$html.="</tr>";
	$html.="<tr>";
	$html.="<td>".get_string('graduatestudentname','block_coach')."</td>";
	$html.="<td><select name='coachees[]' class='chosen-select' multiple data-placeholder='Enter name'>".$graduate_student_options."</select></td>";
	$html.="</tr>";
	$html.="<tr>";
	...
	echo $OUTPUT->header();
	echo $OUTPUT->heading(get_string('graduatestudent:title','block_coach'));
	echo $html;
	echo $OUTPUT->footer();
	echo $html_lib;
}
