...
$PAGE->set_url($CFG->wwwroot. '/blocks/coach/supervisor_new.php');
	...
	if(isset($_POST['coaches'])){
		...
		if(!empty($update)){
			foreach ($update as $key=>$u_id) {
				...
				if(!$DB->record_exists('coach',array('userid'=>$new->userid,'is_student'=>STUDENT_OR_SUPERVISOR))){
					$DB->insert_record('coach',$new);
				}
				$msg.= update_profile_field_value('IsSupervisor',$u_id,MDL_CHECKBOX_CHECK);
				// update_user_position_profile($u_id,POSITION_STUDENTSUPERVISOR);
			}
		}
		if($msg==""){ 
...
$html.=get_string('supervisor:addnew:title','block_coach');

$html.="<form action='supervisor_new.php' method='POST'>";
