...
$coach = optional_param('coach', 0, PARAM_INT); // Delete session.
...
//error_log('>>>>>>>>> USER_TZ = ' . $user_tz);
//error_log('>>>>>>>>> SESSION_TZ = ' . $session_tz);
//error_log('>>>>>>>>> OFFSET = ' . $offset);

$mform = new mod_facetoface_session_form(
  null, 
  compact('id', 'facetoface', 'f', 's', 'c', 'nbdays', 'customfields', 
    'course', 'editoroptions', 'server_tz') //Liberate - removed 'offset' argument as didn't seem to be defined anywhere
);
...
if ($fromform = $mform->get_data()) { // Form submitted.
    ...
    // MA-MODIFIED ===>
    if (empty($fromform->sessiontimezone)) {
        $fromform->sessiontimezone = 'Australia/Sydney';
    }
    ...
    for ($i = 0; $i < $fromform->date_repeats; $i++) {
        if (!empty($fromform->timestart[$i]) and !empty($fromform->timefinish[$i])) {
            $date = new stdClass();
            $date->id = !empty($fromform->sessiondateid[$i]) ? $fromform->sessiondateid[$i] : 0;
            $date->timestart = $fromform->timestart[$i];
            $date->timefinish = $fromform->timefinish[$i];
            $date->is_deleted = !empty($fromform->datedelete[$i]);
            $sessiondates[] = $date;
        }
    }
    
    // MA-MODIFIED ===>
    // Commented. It doesn't seem to calculate correct time
//    foreach ($sessiondates as $key => $sessiondate) {
//
//        $server_tz  = core_date::get_server_timezone();
//        $user_tz    = core_date::get_user_timezone();
//        $session_tz = $fromform->sessiontimezone;
//
//        $sessiondate->timestart = ma_f2f_timestamp_conversion_timezone($sessiondate->timestart, $user_tz, $session_tz);
//        $sessiondate->timefinish = ma_f2f_timestamp_conversion_timezone($sessiondate->timefinish, $user_tz, $session_tz);
//        
//        $sessiondates[$key] = $sessiondate;
//    }
    ...
    $todb->timezone = $fromform->sessiontimezone;   // MA-MODIFIED 
    $todb->additonal_details = $fromform->additional_details;   // MA-MODIFIED
    ...
    $update = false;
    //Copy the original session before it has been modified.
    if($session != null)  $session_previous = clone $session;
    //print_object($sessiondates); die();
    ...
    // file_save_draft_area_files($fromform->sessionimage,$context->id, 'mod_facetoface', 'sessionimage',$session->id,array('subdirs' => true));
    if ($update) {

        // Send update email to attendees
        $attendees = facetoface_get_attendees($session->id);
        //Force attendees to always has ICal attachments.
        // print_object($session_previous);
        //If there is date and it has been changed, then the iCall need to be sent.
        // print_object($session_previous);
        // print_object($sessiondates);
        // die();
        $send_iCal = false;
        if($session_previous->datetimeknown){
            foreach ($session_previous->sessiondates as $key => $date) {
                if(isset($sessiondates[$key])){
                    if($sessiondates[$key]->id!=$date->id) {
                        //No matched the date id => date changed
                        $send_iCal = true;
                        break;
                    }else{
                        if($sessiondates[$key]->timestart!= $session_previous->sessiondates[$key]->timestart || $sessiondates[$key]->timefinish!= $session_previous->sessiondates[$key]->timefinish){
                            //No matched the date time start or time finish => date changed
                            $send_iCal = true;
                            break;
                        }
                    }
        ...
        foreach ($attendees as $attendee) {
            //Force to send new iCal to user
            $attendee->notificationtype = MDL_F2F_ICAL;
            if($send_iCal){
                //facetoface_send_cancellation_notice($facetoface, $session_previous, $attendee->id);
            }
            if ($facetoface->booking_change_notification == 1) {
                facetoface_send_update_notice($facetoface, $session, $attendee->id, $attendee->notificationtype);
            }
        }
    }
    ...
    $toform->additional_details = $session->additonal_details;
    ...
    $toform->sessiontimezone = $session->timezone;  // MA-MODIFIED
    if (has_capability('mod/facetoface:configurecancellation', $context)) {
...
//        $session_tz = $session->timezone;   
        ...
        foreach ($session->sessiondates as $date) {
            ...
            //MA-MODIFIED ===>
//            $date->timestart  = ma_facetoface_offset_timestamp($date->timestart,  $server_tz, $session_tz);
//            $date->timefinish = ma_facetoface_offset_timestamp($date->timefinish, $server_tz, $session_tz);
            //<=== MA-MODIFIED 
    ...
    // $draftid = file_get_submitted_draft_itemid('sessionimage');
    // file_prepare_draft_area($draftid,$context->id, 'mod_facetoface', 'sessionimage', $session->id);
   
    // $toform->sessionimage = $draftid;
