        ...
        foreach ($userstoassign as $adduser) {
            ...
            if (!has_capability('moodle/course:view', $context, $adduser)) {
                ...
                if (!is_enrolled($context, $user)) {
                    ...
                    if (!enrol_try_internal_enrol($course->id, $user->id, $studentroleid)) {    // MA-MODIFIED 
            ...
            if ($existingsignup = $DB->get_record('facetoface_signups', array('sessionid' => $s->id, 'userid' => $adduser))) {
...
$PAGE->set_cm($cm);
// MA-MODIFIED ===>
if($notify==1) $PAGE->set_url('/mod/facetoface/editattendees.php', array('s' => $s, 'backtoallsessions' => $backtoallsessions, 'notify'=>1));
else $PAGE->set_url('/mod/facetoface/editattendees.php', array('s' => $s, 'backtoallsessions' => $backtoallsessions));
// <=== MA-MODIFIED 
...
// MA-MODIFIED ===>
$supp =($suppressemail) ? 1 : 0;

if($notify==1) $url = new moodle_url('/mod/facetoface/attendees.php', array('s' => $session->id, 'backtoallsessions' => $backtoallsessions, 'notify'=>1,'supp'=>$supp));
else $url = new moodle_url('/mod/facetoface/attendees.php', array('s' => $session->id, 'backtoallsessions' => $backtoallsessions,'supp'=>$supp));
// <=== MA-MODIFIED 
