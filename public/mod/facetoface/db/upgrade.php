...
function xmldb_facetoface_upgrade($oldversion=0) {
    ...
    // MA-MODIFIED ===>
    if ($oldversion < 2018031400) {
        // Define field streamlinedaccess to be added to facetoface.
        $table = new xmldb_table('facetoface');
        $field = new xmldb_field('streamlinedaccess', XMLDB_TYPE_INTEGER, '1', null, null, null, '1', 'usercalentry');

        // Conditionally launch add field streamlinedaccess.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        // Facetoface savepoint reached.
        upgrade_mod_savepoint(true, 2018031400, 'facetoface');
    }

    if ($oldversion < 2018032102) {
        $transaction = $DB->start_delegated_transaction();
        $facetoface_events = $DB->get_records('event', array('modulename' => 'facetoface'));

        // Update all events to include whole name not cut-off ones
        foreach ($facetoface_events as $event) {
            $name = $DB->get_field('facetoface', 'name', array('id' => $event->instance));
            if (empty($name)) {
                continue;
            }
            $DB->update_record('event', (object) array(
                'id' => $event->id,
                'name' => $name
            ));
        }
        $transaction->allow_commit();
        upgrade_mod_savepoint(true, 2018032102, 'facetoface');
    }
    // <=== MA-MODIFIED 
    ...
    if ($oldversion < 2018053001) {
        ...
        // Define field waitlist_priority to be added to facetoface_signups_status.
        $table = new xmldb_table('facetoface_signups_status');
        $field = new xmldb_field('waitlist_priority', XMLDB_TYPE_NUMBER, '20, 8', null, null, null, null, 'timecreated');

        // Conditionally launch add field waitlist_priority.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        // Facetoface savepoint reached.
        upgrade_mod_savepoint(true, 2018053001, 'facetoface');
    }
    // <=== MA-MODIFIED 

    // MA-MODIFIED ===>
    if ($oldversion < 2018053003) {
        ...
        $table->add_field('facetoface', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, null);
        $table->add_field('userid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, null);
        $table->add_field('interested', XMLDB_TYPE_INTEGER, '10', null, null, null, '0');
        $table->add_field('emailsent', XMLDB_TYPE_INTEGER, '20', null, null, null, null);
        ...
        // Facetoface savepoint reached.
        upgrade_mod_savepoint(true, 2018053003, 'facetoface');
    }

    if ($oldversion < 2018053004) {

        // Define field timezone to be added to facetoface_sessions.
        $table = new xmldb_table('facetoface_sessions');
        $field = new xmldb_field('timezone', XMLDB_TYPE_CHAR, '50', null, null, null, null, 'timemodified');
        ...
        // Facetoface savepoint reached.
        upgrade_mod_savepoint(true, 2018053004, 'facetoface');
    }
    // <=== MA-MODIFIED 
    ...
    if ($oldversion < 2022112002) {
        ...
        // Define field dietary_requirements to be added to facetoface_sessions.
        $table = new xmldb_table('facetoface_sessions');
        $field = new xmldb_field('dietary_requirements', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0', 'timezone');

        // Conditionally launch add field dietary_requirements.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        // Define field dietary_details to be added to facetoface_sessions.
        $table = new xmldb_table('facetoface_sessions');
        $field = new xmldb_field('dietary_details', XMLDB_TYPE_TEXT, null, null, null, null, null, 'dietary_requirements');

        // Conditionally launch add field dietary_details.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

            // Define field access_requirements to be added to facetoface_sessions.
        $table = new xmldb_table('facetoface_sessions');
        $field = new xmldb_field('access_requirements', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0', 'dietary_details');

        // Conditionally launch add field access_requirements.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

         // Define field access_details to be added to facetoface_sessions.
         $table = new xmldb_table('facetoface_sessions');
         $field = new xmldb_field('access_details', XMLDB_TYPE_TEXT, null, null, null, null, null, 'access_requirements');
 
         // Conditionally launch add field access_details.
         if (!$dbman->field_exists($table, $field)) {
             $dbman->add_field($table, $field);
         }
          // Define field additonal_details to be added to facetoface_sessions.
        $table = new xmldb_table('facetoface_sessions');
        $field = new xmldb_field('additonal_details', XMLDB_TYPE_TEXT, null, null, null, null, null, 'access_details');

        // Conditionally launch add field additonal_details.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        // Facetoface savepoint reached.
        upgrade_mod_savepoint(true, 2022112002, 'facetoface');
    }

    if ($oldversion < 2022112003) {

        // Define field dietary_requirements to be added to facetoface_signups.
        $table = new xmldb_table('facetoface_signups');
        $field = new xmldb_field('dietary_requirements', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0', 'notificationtype');
        ...
        // Define field dietary_details to be added to facetoface_signups.
        $table = new xmldb_table('facetoface_signups');
        $field = new xmldb_field('dietary_details', XMLDB_TYPE_TEXT, null, null, null, null, null, 'dietary_requirements');
        ...
        // Define field access_requirements to be added to facetoface_signups.
        $table = new xmldb_table('facetoface_signups');
        $field = new xmldb_field('access_requirements', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0', 'dietary_details');
        ...
        // Define field access_details to be added to facetoface_signups.
        $table = new xmldb_table('facetoface_signups');
        $field = new xmldb_field('access_details', XMLDB_TYPE_TEXT, null, null, null, null, null, 'access_requirements');
        ...
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
               ...
               // Facetoface savepoint reached.
        upgrade_mod_savepoint(true, 2022112003, 'facetoface');
    }

    if ($oldversion < 2022112004) {

        // Define field formbuilder to be added to facetoface.
        $table = new xmldb_table('facetoface');
        $field = new xmldb_field('formbuilder', XMLDB_TYPE_INTEGER, '10', null, null, null, null, 'streamlinedaccess');

        // Conditionally launch add field formbuilder.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        // Facetoface savepoint reached.
        upgrade_mod_savepoint(true, 2022112004, 'facetoface');
    }

    if ($oldversion < 2023081700) {

        // Define field booking_changemessage to be added to facetoface.
        $table = new xmldb_table('facetoface');
        $field = new xmldb_field('booking_changemessage', XMLDB_TYPE_TEXT, null, null, null, null, null, 'formbuilder');

        // Conditionally launch add field booking_changemessage.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        // Facetoface savepoint reached.
        upgrade_mod_savepoint(true, 2023081700, 'facetoface');
    }

    if ($oldversion < 2025011400) {

        // Define field booking_changesubject to be added to facetoface.
        $table = new xmldb_table('facetoface');
        $field = new xmldb_field('booking_changesubject', XMLDB_TYPE_TEXT, null, null, null, null, null, 'formbuilder');

        // Conditionally launch add field booking_changesubject.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        // Facetoface savepoint reached.
        upgrade_mod_savepoint(true, 2025011400, 'facetoface');
    }

    if ($oldversion < 2025042404) {
        $table = new xmldb_table('facetoface');
    
        // Define field booking_change_notification to be added to facetoface
        $field = new xmldb_field('booking_change_notification', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '0', 'booking_changemessage');

        // Conditionally launch add field booking_change_notification.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        // Facetoface savepoint reached.
        upgrade_mod_savepoint(true, 2025042404, 'facetoface');
    }
