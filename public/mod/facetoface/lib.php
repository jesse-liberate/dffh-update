...
define('CALENDAR_MAX_NAME_LENGTH', 150);  // MA-MODIFIED
...
// MA-MODIFIED ===>
global $FACE_TYPE;
define('FACE_TYPE_CANCEL',              0);
define('FACE_TYPE_REQUEST',             1);
define('FACE_TYPE_REQUEST_APPROVED',    2);
define('FACE_TYPE_CONFIRMED',           3);
define('FACE_TYPE_WAITLISTED',           4);
// <=== MA-MODIFIED 
...
function facetoface_fix_settings($facetoface)
{
        ...
        $facetoface->booking_changemessage = $facetoface->booking_changemessage['text'];
    
   
        $facetoface->remindermessage = $facetoface->remindermessage['text'];
    

        $facetoface->waitlistedmessage = $facetoface->waitlistedmessage['text'];
    
    
        $facetoface->cancellationmessage = $facetoface->cancellationmessage['text'];
    ...
    // MA-MODIFIED ===>
    if (empty($facetoface->streamlinedaccess)) {
        $facetoface->streamlinedaccess = 0;
    }
    if (empty($facetoface->booking_change_notification)) {
        $facetoface->booking_change_notification = 0;
    }
    // <=== MA-MODIFIED 
}
...
function facetoface_add_instance($facetoface)
{
    ...
    // $facetoface->booking_change_notification = isset($facetoface->booking_change_notification) ? 0 : 1;
...
function facetoface_add_session($session, $sessiondates)
{
    ...
    // MA-MODIFIED ===>
    // clear interest if session is upcoming 
    ma_facetoface_clear_interest($session->facetoface);
    // <=== MA-MODIFIED 
...
function facetoface_update_session($session, $sessiondates)
{
    ...
    // We cannot refresh (deleting previous records and inserting again) the sessiondates here.
    // Because if we want to send an update *.ics file to change the event, the IDs must stay
    // the same, therefore deleting sessiondates and inserting them back will chang the IDs,
    // which will then cause any calendar client tool not to update the existing information.
    ...
    if (empty($sessiondates)) {
        ...
        foreach ($sessiondates as $key => $date) {
            if ($date->is_deleted) {
                if (!empty($date->id)) {
                    $DB->delete_records('facetoface_sessions_dates', ['id' => $date->id]);
                }
                unset($sessiondates[$key]);
                continue;
            }
            ...
            if (!empty($date->id)) {
                ...
                $sessiondates[$key]->id = $DB->insert_record('facetoface_sessions_dates', $date);
            }
        }
    }
...
function facetoface_update_attendees($session)
{
    ...
    // MA-MODIFIED ===>
    // sort by waitlist_priority, lower value higher priority
    usort($users, function ($a, $b) {
        return ($a->waitlist_priority > $b->waitlist_priority);
    });
    // <=== MA-MODIFIED 
 ...
 * @return  string
 */
 function facetoface_email_substitutions($msg, $facetofacename, $reminderperiod, $user, $data, $sessionid,$coursename) {// MA-MODIFIED
    global $CFG, $DB;
    ...
    if ($data->datetimeknown) {
        ...
        $session_tz = $data->timezone;
        //$user_tz    = core_date::get_user_timezone();
        $dateformat = get_string('formatsessiondate', 'mod_facetoface');
        $timeformat = get_string('formatsessiontime', 'mod_facetoface');
        //$timezone   = '';
        //End of Mindatlas
        ...
        $start_time  = new timezone_class($data->sessiondates[0]->timestart, $user->timezone, $session_tz);
        $finish_time = new timezone_class($data->sessiondates[0]->timefinish, $user->timezone, $session_tz);
        $sessiondate = $start_time->getUsertime($dateformat);
        $starttime   = $start_time->getUsertime($timeformat);
        $finishtime  = $finish_time->getUsertime($timeformat);

        //        $sessiondate = ma_facetoface_get_date_format(
        //          $server_tz, 
        //          ma_facetoface_get_adjusted_timestamp(
        //            $data->sessiondates[0]->timestart, 
        //            $session_tz, 
        //            $user->timezone
        //          ),
        //          $dateformat
        //        );
        //        $starttime = ma_facetoface_get_date_format(
        //          $server_tz, 
        //          ma_facetoface_get_adjusted_timestamp(
        //            $data->sessiondates[0]->timestart, 
        //            $session_tz, 
        ...
        //          $timeformat
        //        );
        //        $finishtime = ma_facetoface_get_date_format(
        //          $server_tz,
        //          ma_facetoface_get_adjusted_timestamp(
        //            $data->sessiondates[0]->timefinish, 
        //            $session_tz, 
        //            $user->timezone
        //          ),
        //          $timeformat
        ...
        foreach ($data->sessiondates as $date) {
            ...
            // mindatlas changes start        
            $session_start  = new timezone_class($date->timestart, $user->timezone, $session_tz);
            $session_finish = new timezone_class($date->timefinish, $user->timezone, $session_tz);

            $user_datestart  = $session_start->getUsertime($dateformat);
            $user_timestart  = $session_start->getUsertime($timeformat);
            $user_timefinish = $session_finish->getUsertime($timeformat);
            $session_timestart = $session_start->getSessiontime($timeformat);
            $session_timefinish = $session_finish->getSessiontime($timeformat);

            //            $user_datestart = ma_facetoface_get_date_format(
            //              $server_tz, 
            //              ma_facetoface_get_adjusted_timestamp($date->timestart, $session_tz, $user->timezone),
            //              $dateformat
            //            );
            //            $user_timestart = ma_facetoface_get_date_format(
            //              $server_tz, 
            //              ma_facetoface_get_adjusted_timestamp($date->timestart, $session_tz, $user->timezone),
            //              $timeformat
            //            );
            //            $user_timefinish = ma_facetoface_get_date_format(
            //              $server_tz,
            //              ma_facetoface_get_adjusted_timestamp($date->timefinish, $session_tz, $user->timezone),
            //              $timeformat
            //            );
            //            $session_timestart = ma_facetoface_get_date_format(
            //              $server_tz,
            //              $date->timestart,
            //              $timeformat
            //            );
            //            $session_timefinish = ma_facetoface_get_date_format(
            //              $server_tz, 
            //              $date->timefinish,
            //              $timeformat
            //            );
            $alldates .= $user_datestart . ', '
                . $user_timestart . ' to '
                . $user_timefinish . ' ('
                . $session_tz . ' : '
                . $session_timestart . ' - '
                . $session_timefinish . ')';
            // mindatlas changes end
            ...
            //            //MindAtlas
            //            $timestart_stamp  = ma_facetoface_offset_timestamp($date->timestart,  $server_tz, $session_tz);
            //            $timefinish_stamp = ma_facetoface_offset_timestamp($date->timefinish, $server_tz, $session_tz);
            //            $timestart  = date('g:i A', $timestart_stamp);
            //            $timefinish = date('g:i A', $timefinish_stamp);
            //            $alldates .= " ( $session_tz : $timestart &ndash; $timefinish )";
            // $alldates .= html_writer::empty_tag('br');
            //MindAtlas
    ...
    $facetofacename = get_string('course_module_name', 'facetoface', (object)['modulename' => $facetofacename, 'coursename' => $coursename]); // MA-MODIFIED
    ...
    $msg = str_replace(get_string('placeholder:support_contact_name', 'facetoface'), fullname($support_user), $msg);
    $msg = str_replace(get_string('placeholder:support_contact_email', 'facetoface'), $support_user->email, $msg);

    if (!empty($user->manager)) {
        $msg = str_replace(get_string('placeholder:manager_firstname', 'facetoface'), $user->manager->firstname, $msg);
    }

    // $url = $CFG->wwwroot."/mod/facetoface/cancelsignup.php?s=".$sessionid;
    // $cancel_link = get_string('cancel_session_link','facetoface',$url);
    // $msg = str_replace(get_string('placeholder:cancelsessionlink', 'facetoface'), $cancel_link, $msg);
    // <=== MA-MODIFIED 
    ...
    $attendees_link = "<a href='" . $CFG->wwwroot . '/mod/facetoface/attendees.php?s=' . $sessionid . "#unapproved'>" . $CFG->wwwroot . '/mod/facetoface/attendees.php?s=' . $sessionid . "#unapproved</a>";
    $msg = str_replace(get_string('placeholder:attendeeslink', 'facetoface'), $attendees_link, $msg);
...
function facetoface_cron()
{
    ...
    foreach ($signupsdata as $signupdata) {
        ...
        // MA-MODIFIED ===>
        //MINDATLAS - if the session detail is not showing, the intro will be used
        if (trim(strip_tags($signupdata->details)) == "") {
            $signupdata->details = $signupdata->intro;
        }
        // <=== MA-MODIFIED 
        ...
        $posthtml = $posttext; // FIXME.// MA-MODIFIED
        ...
        // MA-MODIFIED ===>
        require_once($CFG->dirroot . '/admin/tool/delayemail/lib.php');
        //MINDATLAS
        $mynotification = false;
        if ($DB->record_exists('config_plugins', array('plugin' => 'block_mynotification', 'name' => 'version'))) {
            require_once($CFG->dirroot . '/blocks/mynotification/lib.php');
            $mynotification = true;
            $short_content = "<span>" . $postsubject . "</span>";
            add_to_mynotification($user->id, $short_content, $posthtml);
        }
        // <=== MA-MODIFIED 

        if (email_to_user_delay($user, $from, strip_tags($postsubject), strip_tags($posttext), nl2br($posthtml))) { // MA-MODIFIED
            ...
            //MINDATLAS
            if ($mynotification) {
                $manager_id = get_hierarchy_manager_id($user->id, $manager->email);
                if (is_array($manager_id)) {
                    foreach ($manager_id as $mgr_id) {
                        add_to_mynotification($user->id, $short_content, $posthtml, $mgr_id);
                    }
                } else add_to_mynotification($user->id, $short_content, $posthtml, $manager_id);
            }
            // <=== MA-MODIFIED 
            // Send email to mamager.
            if (email_to_user_delay($manager, $from, strip_tags($postsubject), strip_tags($managertext), nl2br($posthtml))) { // MA-MODIFIED
...
function facetoface_get_userfields()
{
    ...
    if (null == $userfields) {
        ...
        if (function_exists('grade_export_user_fields')) {
            ...
            //MindAtlas - Only show relevant profile fields in Export Attendance.
            ...
            //MindAtlas - Add Position Title to Export Attendance
            // $userfields['PositionTitle'] = 'Position Title';    // MA-MODIFIED
            //MindAtlas 20250428 - Add following to Export Attendance
            $userfields['fullname'] = 'Full name';
            $userfields['email'] = 'Email address';
            $userfields['OrganisationOrAgency'] = 'Organisation';
            $userfields['RoleOrPosition'] = 'Role';
...
function facetoface_write_worksheet_header(&$worksheet)
{
    ...
    $worksheet->write_string(0, $pos++, 'Training module');
    ...
    $worksheet->write_string(0, $pos++, 'Date of session');
...
function facetoface_write_activity_attendance(
    ...
    foreach ($sessions as $session) {
        ...
        $activityname = '-';
        if (isset($facetofaceid)) {
            $activityname = $DB->get_field('facetoface', 'name', array('id' => $facetofaceid));
        }
        ...
        if (!empty($sessionsignups[$session->id])) {
            foreach ($sessionsignups[$session->id] as $attendee) {
                ...
                // Add activity name column - fetch from database if necessary
                if (isset($facetofaceid)) {
                ...
                foreach ($userfields as $shortname => $fullname) {
                    ...
                    if ($shortname == 'fullname') {
                        $worksheet->write_string($i, $j++, $attendee->firstname . ' ' . $attendee->lastname);
                    }
                ...
                if (empty($sessiondate)) {
                    ...
                    if (method_exists($worksheet, 'write_date')) {
                        $formatted_date = userdate($sessiondate, '%d/%m/%Y');
                        $worksheet->write_string($i, $j++, $formatted_date);
                    } else {
...
function mod_facetoface_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options=array()) {
    ...
    $fullpath = "/$context->id/mod_facetoface/$filearea/$contentid/$relativepath";
    ...
    send_stored_file($file, 60*60*6, 0, true, $options); // download MUST be forced - security!

    return false;
}

function facetoface_get_editor_options($context) {
    global $CFG;
...
{
    global $CFG, $DB;
    // MA-MODIFIED: add f.intro as intro, between f.name as facetofacename, and f.reminderperiod,
...
function facetoface_update_sessionuser($session){
    global $USER, $DB;
...
function facetoface_user_signup(
    ...
    $userid = false,
    $notifyuser = true,
    $chosen_manageremail = null
) {    // MA-MODIFIED

    global $CFG, $DB, $FACE_TYPE;    // MA-MODIFIED
    ...
    if( $session->dietoptions){
        $usersignup->dietary_details =  $session->diettext;
        $usersignup->access_details =  $session->accesstext;
        if($session->dietoptions == 'yes'){
            $usersignup->dietary_requirements = 1;
        }else{
            $usersignup->dietary_requirements = 0;
            $usersignup->dietary_details =  '';
        }
        if(  $session->accessOptions == 'yes'){
            $usersignup->access_requirements = 1;
        }else{
            $usersignup->access_requirements = 0;
            $usersignup->access_details =  '';
        }
    } 
    ...
    if (!$facetoface->approvalreqd ) {
        ...
        if ($currentstatus == MDL_F2F_STATUS_APPROVED) {
            ...
            $FACE_TYPE = FACE_TYPE_CONFIRMED;   // MA-MODIFIED
            $newstatus = MDL_F2F_STATUS_REQUESTED;
        } else {
            $FACE_TYPE = FACE_TYPE_REQUEST; // MA-MODIFIED
    ...
    if ($notifyuser) {
        ...
        switch ($newstatus) {
            ...
            case MDL_F2F_STATUS_REQUESTED:
                $error = facetoface_send_request_notice($facetoface, $session, $userid, $chosen_manageremail);  // MA-MODIFIED
                break;
        }
...
// MA-MODIFIED ===>
function facetoface_user_team_signup(
    $session,
    $facetoface,
    $course,
    $discountcode,
 ...
 */
function facetoface_send_request_notice($facetoface, $session, $userid, $chosen_manageremail)
{ // MA-MODIFIED
    global $DB, $CFG, $FACE_TYPE; // MA-MODIFIED
    ...
    if (empty($chosen_manageremail)) {
        $manager = get_first_manager($user->id);    // MA-MODIFIED
    } else {
        $manager = core_user::get_user_by_email($chosen_manageremail);
    }

    $user->manager = $manager;

    $coursename = $DB->get_field('course', 'shortname', array('id' => $facetoface->course));    // MA-MODIFIED
    $postsubject = facetoface_email_substitutions(
        ...
        $user,
        $session,
        $session->id,   // MA-MODIFIED
        $coursename     // MA-MODIFIED
    );
    ...
    require_once($CFG->dirroot . '/admin/tool/delayemail/lib.php');   // MA-MODIFIED
    // Send to user.
    if (!email_to_user_delay($user, $from, strip_tags($postsubject), strip_tags($posttext), nl2br($posttext))) {    // MA-MODIFIED
        return 'error:cannotsendrequestuser';
    }

    // Send to manager.
    $user = $manager;    // MA-MODIFIED
    ...
    if ($DB->record_exists('config_plugins', array('plugin' => 'block_mynotification', 'name' => 'version'))) {
        require_once($CFG->dirroot . '/blocks/mynotification/lib.php');
        $mynotification = true;
        // $short_content = get_string('notification:requestmessage','facetoface',(object)['activityname'=>$facetoface->name,'coursename'=>$coursename]);

        $short_content = "<span>" . $postsubject . "</span> "; //.$short_content;
        $cancel_link = $CFG->wwwroot . "/mod/facetoface/cancelsignup.php?s=" . $session->id . "&backtoallsessions=" . $facetoface->id;
        $cancel_link_str = get_string('cancel_session_link', 'facetoface', $cancel_link);
        add_to_mynotification($user->id, $short_content . $cancel_link_str, $posttext);
        $manager_ids = get_hierarchy_manager_id_levels($user->id, $chosen_manageremail);

        if (is_array($manager_ids)) {
            foreach ($manager_ids as $mgr_id) {
                add_to_mynotification($user->id, $short_content, $posttext, $mgr_id);
            }
        } else add_to_mynotification($user->id, $short_content, $posttext, $manager_ids);


        if ($FACE_TYPE == FACE_TYPE_CONFIRMED) {
            $url = $CFG->wwwroot . "/mod/facetoface/attendees.php?notify=1&s=" . $session->id;
            $short_content = get_string('notification:confirmmanager', 'facetoface', (object)['url' => $url, 'activityname' => $facetoface->name, 'coursename' => $coursename]);
            $manager_link = '                
                Click on <a href="' . $url . '">the link</a> to approve, decline or decide later the <strong>' . $facetoface->name . '</strong> (<strong>' . $coursename . '</strong> training)';

            if (is_array($manager_ids)) {
                foreach ($manager_ids as $mgr_id) {
                    $manager = $DB->get_record('user', array('id' => $mgr_id));
                    $a = new stdClass();
                    $a->fullname = fullname($manager); //ucfirst($manager->firstname)." ".ucfirst($manager->lastname);
                    $a->content = substr($posttext, strpos($posttext, 'Participant'));
                    $manager_html = get_string('notification:confirmmanagercontent', 'facetoface', $a);
                    add_to_mynotification($user->id, $short_content, $manager_html, $mgr_id);
                }
            } else {
                $manager = $DB->get_record('user', array('id' => $manager_ids));
                $a = new stdClass();
                $a->fullname = fullname($manager); //ucfirst($manager->firstname)." ".ucfirst($manager->lastname);
                $a->content = substr($posttext, strpos($posttext, 'Participant'));
                $manager_html = get_string('notification:confirmmanagercontent', 'facetoface', $a);
                add_to_mynotification($user->id, $short_content, $manager_html, $manager_ids);
            }
        }
    }
    if ($FACE_TYPE == FACE_TYPE_CONFIRMED) {
        if (!email_to_user_delay($user, $from, strip_tags($postsubject), strip_tags($posttextmgrheading), nl2br($posttextmgrheading))) {
...
// MA-MODIFIED ===>
function get_first_manager_email($userid)
{
    global $DB;

    $sql = "SELECT hu1.user_id from mdl_hierarchy_user as hu1 where hu1.node_id in (select hn.parent_node_id from mdl_hierarchy_node as hn inner join mdl_hierarchy_user hu2 on hu2.node_id = hn.id and hu2.user_id=?)";
    $rs = $DB->get_records_sql($sql, array($userid));
    if (!empty($rs)) {
        foreach ($rs as $row) {
            $user = $DB->get_record('user', array('id' => $row->user_id));
            return $user->email;
        }
    }
}
function get_first_manager($userid)
{
    global $DB;

    $sql = "SELECT u.*
        FROM {hierarchy_user} as hu
        JOIN {hierarchy_node} as hn
        ON hn.id = hu.node_id
        JOIN {hierarchy_user} as manager_user
        ON manager_user.node_id = hn.parent_node_id
        JOIN {user} u
        ON u.id = manager_user.user_id
        WHERE u.deleted = 0 AND u.suspended = 0 AND u.confirmed = 1  AND hu.user_id = ?
        LIMIT 1";
...
function facetoface_update_signup_status($signupid, $statuscode, $createdby, $note = '', $grade = null)
{
    ...
    $signupstatus->waitlist_priority = time();
 ...
 */
function facetoface_send_notice_original($postsubject, $posttext, $posttextmgrheading,
        ...
        if (get_config(null, 'facetoface_oneemailperday')) {
            ...
            foreach ($sessiondates as $sessiondate) {
                ...
                $body = facetoface_email_substitutions($posttext, format_string($facetoface->name), $facetoface->reminderperiod,
                                                       $user, $session, $session->id,$coursename);
                $htmlmessage = facetoface_email_substitutions($posttext, $facetoface->name, $facetoface->reminderperiod, $user, $session, $session->id,$coursename);
                $htmlbody = $htmlmessage;
                $icalattachments[] = array('filename' => $filename, 'subject' => $subject,
                                           'body' => $body, 'htmlbody' => $htmlbody,'timestart' => $timestart);
            }
    ...
    if ($notificationtype && MDL_F2F_ICAL) {
        foreach ($icalattachments as $attachment) {
            $attachment['subject'] = strip_tags($attachment['subject']);
            
            if (!ma_email_to_user($user, $from, $attachment['subject'],$posttext,
            $posttext, $attachment['filename'], $attachmentfilename)) {
    ...
    if (!empty($facetoface->thirdparty) &&
        ...
        foreach ($recipients as $recipient) {
            if($user->email != $recipient){
                ...
                // Temporary thirdparty duplicate email fix
...
function facetoface_send_notice(
    ...
    // MA-MODIFIED ===>
    //MINDATLAS - if the session detail is not showing, the intro will be used
    if (isset($session->details) && trim(strip_tags($session->details)) == "") {
        $session->details = $facetoface->intro;
    }
    // <=== MA-MODIFIED 
    ...
    if ($notificationtype & MDL_F2F_ICAL) {
        if ($notificationtype & MDL_F2F_INVITE) {
            $attachmentfilename = 'invite';
        } else if ($notificationtype & MDL_F2F_CANCEL) {
            $attachmentfilename = 'cancel';
        }
    }
    $coursename = $DB->get_field('course', 'shortname', array('id' => $facetoface->course));    // MA-MODIFIED 
    // Do iCal attachement stuff.
    ...
    if ($notificationtype & MDL_F2F_ICAL) {
        if (get_config(null, 'facetoface_oneemailperday')) {
            ...
            foreach ($sessiondates as $sessiondate) {
                ...
                $body = facetoface_email_substitutions(
                    $posttext,
                    $facetoface->name,
                    $facetoface->reminderperiod,
                    $user,
                ...
                $htmlbody = nl2br($body); // TODO.  // MA-MODIFIED 
                $icalattachments[] = array(
            ...
            $htmlbody = nl2br($body); // FIXME. // MA-MODIFIED 
            $icalattachments[] = array(
                'filename' => $filename, 'subject' => $subject,
                'body' => $body, 'htmlbody' => $htmlbody
            );
    ...
    $posthtml = nl2br($posttext); // FIXME. // MA-MODIFIED 
    if ($fromaddress = get_config(null, 'facetoface_fromaddress')) {
        $from = new stdClass();
        $from->maildisplay = true;
        $from->email = $fromaddress;
    ...
    if ($notificationtype & MDL_F2F_ICAL) {
        foreach ($icalattachments as $attachment) {
            ...
            if (isset($attachment['filename'])) {
                $attachment_paths = $attachment['filename'];
                $attachment_names = array();
                for ($i = 0; $i < count($attachment_paths); $i++) {
                    $attachment_names[$i] = $attachmentfilename . ($i + 1) . ".ics";
                }
                if (!email_to_user_delay(
                    $user,
                    $from,
                    strip_tags($attachment['subject']),
                    strip_tags($attachment['body']), // MA-MODIFIED
                    $attachment['htmlbody'],
                    $attachment_paths,
                    $attachment_names
                )) { // MA-MODIFIED
    ...
    } elseif ($notificationtype & MDL_F2F_TEXT) { // Send plain text email.  // MA-MODIFIED
        if (!email_to_user_delay($user, $from, strip_tags($postsubject), strip_tags($posttext), $posthtml)) {   // MA-MODIFIED
            return 'error:cannotsendconfirmationuser';
        }
    }

    // Manager notification.
    $manageremail = $chosen_manageremail;   // MA-MODIFIED
    if (!empty($posttextmgrheading) and !empty($manageremail) and $session->datetimeknown) {
        ...
        // Leave out the ical attachments in the managers notification.
        if (!email_to_user_delay($manager, $from, strip_tags($postsubject), strip_tags($managertext), $posthtml)) { // MA-MODIFIED
            return 'error:cannotsendconfirmationmanager';
        }
    }
    ...
    if ($DB->record_exists('config_plugins', array('plugin' => 'block_mynotification', 'name' => 'version'))) {
        require_once($CFG->dirroot . '/blocks/mynotification/lib.php');
        $mynotification = true;
        $coursename = $DB->get_field('course', 'shortname', array('id' => $facetoface->course));
        // echo "<pre>".print_r($notificationtype,true)."</pre>";
        $short_content = "";
        $short_content_mgr = "";
        switch ($FACE_TYPE) {
            case FACE_TYPE_CANCEL:
                // var_dump($FACE_TYPE);die();
                $session_datetime = get_session_date_time($session);
                $short_content = get_string('notification:cancelrequestmessage', 'facetoface', (object)['activityname' => $facetoface->name, 'coursename' => $coursename, 'datetime' => $session_datetime]);
                $short_content = "<span>" . $postsubject . "</span>: " . $short_content;
               
                add_to_mynotification($userid, $short_content, $posttext);
                break;
            case FACE_TYPE_CONFIRMED:
                $url = $CFG->wwwroot . "/mod/facetoface/attendees.php?notify=1&s=" . $session->id;
                $short_content_mgr = get_string('notification:confirmmanager', 'facetoface', (object)['url' => $url, 'activityname' => $facetoface->name, 'coursename' => $coursename]);
                //short_content
                // $session_datetime = get_session_date_time($session);
                $postsubject = str_replace("[coursename]", $coursename, $postsubject);
                $short_content = "<span>" . $postsubject . "</span>";
                // var_dump($FACE_TYPE.'2');die();
                add_to_mynotification($userid, $short_content, $posttext);
                break;
            case FACE_TYPE_WAITLISTED:
                $url = $CFG->wwwroot . "/mod/facetoface/attendees.php?notify=1&s=" . $session->id;
                $short_content_mgr = get_string('notification:confirmmanager', 'facetoface', (object)['url' => $url, 'activityname' => $facetoface->name, 'coursename' => $coursename]);
                //short_content
                $cancel_link = $CFG->wwwroot . "/mod/facetoface/cancelsignup.php?s=" . $session->id;
                $cancel_link_str = get_string('cancel_session_link', 'facetoface', $cancel_link);
                $postsubject = str_replace("[coursename]", $coursename, $postsubject);
                $short_content = "<span>" . $postsubject . "</span>";
                // var_dump($FACE_TYPE.'3');die();
                add_to_mynotification($userid, $short_content . $cancel_link_str, $posttext);
                break;
            case FACE_TYPE_REQUEST_APPROVED:
                $short_content = get_string('notification:approvedmessage', 'facetoface', (object)['activityname' => $facetoface->name, 'coursename' => $coursename]);
                $short_content = "<span>" . $postsubject . "</span>: " . $short_content;
                $cancel_link = $CFG->wwwroot . "/mod/facetoface/cancelsignup.php?s=" . $session->id;
                $cancel_link_str = get_string('cancel_session_link', 'facetoface', $cancel_link);
                // var_dump($FACE_TYPE.'5');die();
                add_to_mynotification($user->id, $short_content . $cancel_link_str, $posttext);
                break;
        }
        $manager_id = get_hierarchy_manager_id($user->id, $manageremail);
        if ($short_content_mgr != "") $short_content = $short_content_mgr;
        if (is_array($manager_id)) {
            foreach ($manager_id as $mgr_id) {
                add_to_mynotification($user->id, $short_content, $posttext, $mgr_id);
            }
        } else add_to_mynotification($user->id, $short_content, $posttext, $manager_id);
    }
    // <=== MA-MODIFIED 
    ...
    if (
        ...
        foreach ($recipients as $recipient) {
            ...
            // Leave out the ical attachments in the 3rd parties notification.
            if (!email_to_user_delay($thirdparty, $from, strip_tags($postsubject), strip_tags($posttext), $posthtml)) { // MA-MODIFIED
                return 'error:cannotsendconfirmationthirdparty';
            }
        }
    }
...
// MA-MODIFIED ===>
function get_session_date_time($data)
{
    if ($data->datetimeknown) {
        // Scheduled session.
        $sessiondate = userdate($data->sessiondates[0]->timestart, get_string('strftimedate'));
    ...
    $html = $starttime . "-" . $finishtime . " , " . $sessiondate;
    return $html;
}

function facetoface_send_update_notice($facetoface, $session, $userid, $notificationtype)
{
    global $FACE_TYPE;  // MA-MODIFIED
    $posttextmgrheading = false; // Empty so that it does not send email to managers

    if ($facetoface->booking_change_notification == 0) {
        return ''; // Skip sending notification
    }

    if (empty($facetoface->booking_changesubject)) {
        $postsubject = get_string('setting:defaultbooking_changesubjectdefault', 'facetoface');
        return '';
    } else {
        $postsubject = $facetoface->booking_changesubject;
    }
    if (empty($facetoface->booking_changemessage)) {
        $posttext = get_string('setting:defaultbooking_changemessagedefault', 'facetoface');
    } else {
        $posttext = $facetoface->booking_changemessage;
    }

    // Set update bit.
    $notificationtype |= MDL_F2F_UPDATE;
    // Set invite bit.
    $notificationtype |= MDL_F2F_INVITE;

    return facetoface_send_notice_original(
        $postsubject,
        $posttext,
        $posttextmgrheading,
        $notificationtype,
...
function facetoface_send_confirmation_notice($facetoface, $session, $userid, $notificationtype, $iswaitlisted)
{
    ...
    if (!$iswaitlisted) {
        $FACE_TYPE = FACE_TYPE_REQUEST_APPROVED;    // MA-MODIFIED
        $postsubject = $facetoface->confirmationsubject;
        $posttext = $facetoface->confirmationmessage;
    } else {
        $FACE_TYPE = FACE_TYPE_WAITLISTED;  // MA-MODIFIED
...
{
    global $DB, $FACE_TYPE;  // MA-MODIFIED
 ...
 */
//MINDATLAS: using hierarchy is the feature is installed
function facetoface_get_manageremail($userid) {
    global $DB;
    if($DB->record_exists('config_plugins',array('plugin'=>'tool_hierarchy','name'=>'version'))){
        if(is_siteadmin($userid)){
            return '';
        }else{
            $sql="SELECT hu1.user_id from mdl_hierarchy_user as hu1 where hu1.node_id in (select hn.parent_node_id from mdl_hierarchy_node as hn inner join mdl_hierarchy_user hu2 on hu2.node_id = hn.id and hu2.user_id=?)";
            $manager_rs = $DB->get_records_sql($sql,array($userid));
            // var_dump($manager_rs);die();
            if(!empty($manager_rs)){
                foreach ($manager_rs as $row) {
                    return $DB->get_field('user','email',array('id'=>$row->user_id));// Return first manager's email
                    break;
                }
            }else{// Get higher level
                $sql = 'SELECT hu1.user_id from mdl_hierarchy_user as hu1 where hu1.node_id in (select parent_node_id from mdl_hierarchy_node where id in( select hn.parent_node_id from mdl_hierarchy_node as hn inner join mdl_hierarchy_user hu2 on hu2.node_id = hn.id and hu2.user_id=?))';
                $rs = $DB->get_records_sql($sql,array($userid));
                if(!empty($rs)){
                    foreach ($rs as $row) {
                        return $DB->get_field('user','email',array('id'=>$row->user_id));// Return first manager's email
...
function facetoface_take_attendance($data)
{
    ...
    foreach ($data as $key => $value) {
        // MA-MODIFIED ===>        
        $subidcheck = substr($key, 0, 6);

        if ($subidcheck == 'subid_') {

            $submissionid = substr($key, 6);
            $isattended = 'isattended_' . $submissionid;
            $comments = 'commentid_' . $submissionid;
            $note = isset($data->$comments) ? $data->$comments : '';
        
            if (isset($data->$isattended) && $note != 'partially_attended_one' &&  $note != 'partially_attended_two') {
                $grade = 100;
                
                $value2 = MDL_F2F_STATUS_FULLY_ATTENDED;
            } else if ($note == 'partially_attended_one' || $note == 'partially_attended_two'){
                $grade = 50;
                $value2 = MDL_F2F_STATUS_PARTIALLY_ATTENDED;
            }else{
                $grade = 0;
                $value2 = MDL_F2F_STATUS_NO_SHOW;
            }

           
          
            facetoface_update_signup_status($submissionid, $value2, $USER->id, $note, $grade);
            if (!facetoface_take_individual_attendance($submissionid, $grade)) {
                error_log("F2F: could not mark '$submissionid' as " . $value2);
                return false;
            }
            // Update completion status for the student
            face_to_face_update_completion($submissionid, $grade);
        }
        // <=== MA-MODIFIED 
    }
...
// MA-MODIFIED ===>
// Update the module completion or not completed based on the Grade of student.
// Student will be marked as complete when they are fully attended
function face_to_face_update_completion($signupid, $grade = 0)
{
    global $DB, $CFG;
    $session = $DB->get_record('facetoface_signups', array('id' => $signupid));
    $sessionid = $session->sessionid;
    $userid = $session->userid;
    $facetofaceid = $DB->get_field('facetoface_sessions', 'facetoface', array('id' => $sessionid));
    $courseid = $DB->get_field('facetoface', 'course', array('id' => $facetofaceid));
    $mod_id = $DB->get_field('modules', 'id', array('name' => 'facetoface'));
    $course_module_id = $DB->get_field('course_modules', 'id', array('course' => $courseid, 'module' => $mod_id, 'instance' => $facetofaceid));
    $record = $DB->get_record('course_modules_completion', array('coursemoduleid' => $course_module_id, 'userid' => $userid));
    if (!empty($record)) {
        $record->completionstate = ($grade == 100) ? 1 : 0;
        $DB->update_record('course_modules_completion', $record);
    } else {
        $new = new stdClass();
        $new->coursemoduleid = $course_module_id;
        $new->userid = $userid;
        $new->completionstate = ($grade == 100) ? 1 : 0;
        $new->timemodified = time();
        $new->viewd = 0;
        $DB->insert_record('course_modules_completion', $new);
    }
}
// <=== MA-MODIFIED 
...
function facetoface_approve_requests($data)
{
    ...
    $suppressemail = (!isset($data->suppressemail)) ? false : $data->suppressemail; // MA-MODIFIED
        ...
        switch ($value) {
            ...
            case 1:
                ...
                // MA-MODIFIED ===>
                    ...
                    facetoface_user_cancel($session, $attendee->id, true);
            ...
            case 2:
                ...
                if (!facetoface_user_signup(
                    ...
                    $status,
                    $attendee->id,      // MA-MODIFIED
                    true,    // MA-MODIFIED
                    $chosen_manageremail = null   // MA-MODIFIED
                )) {
                    //Liberate update - changed to continue 2 as per php warning recommendation
                    continue 2;
                }
 ...
 */
function facetoface_format_session_times(
    $start,
    $end,
    $tz,
    $dateformat = null,
    ...
    $obj = new stdClass();
    $server_tz = core_date::get_server_timezone();
    $obj->startdate = ma_facetoface_get_date_format($server_tz, $start, $dateformat);
    $obj->enddate   = ma_facetoface_get_date_format($server_tz, $end, $dateformat);
    $obj->starttime = ma_facetoface_get_date_format($server_tz, $start, $timeformat);
    $obj->endtime   = ma_facetoface_get_date_format($server_tz, $end, $timeformat);
    $obj->timezone  = $tz;
    return $obj;
    /**
     * Commented out
     * The old code doesn't seem to work as required
     * It's now replaced by codes above
    ...
    $formattedsession->startdate = userdate($start, $dateformat, $targetTZ);
    $formattedsession->starttime = userdate($start, $timeformat, $targetTZ);
    $formattedsession->enddate = userdate($end, $dateformat, $targetTZ);
    $formattedsession->endtime = userdate($end, $timeformat, $targetTZ);
    ...
    if ($submissions = facetoface_get_user_submissions($facetoface->id, $USER->id)) {
        ...
        foreach ($submissions as $submission) {
            ...
            if ($session = facetoface_get_session($submission->sessionid)) {
                ...
                } else if (facetoface_is_user_awaiting_approval($session, $USER->id)) {
                    $status = get_string('awaitingapproval', 'facetoface');
                } else {
                ...
                if ($session->datetimeknown) {
                    foreach ($session->sessiondates as $date) {
                        ...
                        $sessionobj = facetoface_format_session_times($date->timestart, $date->timefinish, $session->timezone);
                ...
                $sessiondates = str_replace(" (time", "    (Time", $sessiondates);
            ...
            foreach ($sessions as $session) {
                ...
                if ($session->datetimeknown) {
                    if (empty($session->sessiondates)) {
                        ...
                        $sessionobj = facetoface_format_session_times($session->sessiondates[0]->timestart, $session->sessiondates[0]->timefinish, $session->timezone);
            ...
            if (!empty($sessionsinprogress)) {
                ...
                foreach ($sessionsinprogress as $session) {
                    $output .= html_writer::start_tag('div', array('class' => 'f2fsession f2finprogress'))
                        . html_writer::tag('span', $session->date . $session->multidate . ' ' . $session->timezone, array('class' => 'f2fsessiontime'))
                        . html_writer::end_tag('div');
                }
            ...
            if (!empty($futuresessions)) {
                ...
                foreach ($futuresessions as $session) {
                    ...
                    $output .= html_writer::start_tag('div', array('class' => 'f2fsession f2ffuture'))
                        . html_writer::tag('div', $session->date . $session->multidate . ' ' . $session_timezone, array('class' => 'f2fsessiontime'))
...
function facetoface_get_ical_attachment($method, $facetoface, $session, $user)
{
    ...
    $filenames = array();
    //MA - MODIFIED
    $coursename = $DB->get_field('course', 'fullname', array('id' => $facetoface->course));
    $session_detail = "Course: " . $coursename . "<br>\n";
    // MA-MODIFIED ===>
    foreach ($session->sessiondates as $date) {
        $timestart  = ma_facetoface_get_date_format(null, $date->timestart, 'd/m/Y g:i A');
        $timefinish = ma_facetoface_get_date_format(null, $date->timefinish, 'd/m/Y  g:i A');
        $timezone = "$session_tz ($timestart &ndash; $timefinish) " . "<br>\n";

        $session_detail .= $timezone;
    }
    ...
    foreach ($session->sessiondates as $date) {
        ...
        $current_sequence = 1;
        if (($method & MDL_F2F_UPDATE)) {
            $previous_sequence = facetoface_get_previous_sequence($facetoface, $session, $user);
            $current_sequence = $previous_sequence + 1;
        }
        facetoface_save_sequence($facetoface, $session, $user, $current_sequence);
        $SEQUENCE = $current_sequence;

        $SUMMARY     = facetoface_ical_escape($facetoface->name);

        $DESCRIPTION = facetoface_ical_escape($session_detail, true);
        //Add some cancelation link into the Description
        // <=== MA-MODIFIED 

        // Get the location data from custom fields if they exist.
        $customfielddata_sensitive = facetoface_get_customfielddata($session->id);      // MA-MODIFIED
        $customfielddata = array_change_key_case($customfielddata_sensitive, CASE_LOWER); // MA-MODIFIED
        $locationstring = '';
...
EOF;
        ...
        $tempfilename = $user->id . $session->id . md5($template);
...
function facetoface_ical_escape($text, $converthtml = false)
{
    ...
    if (!$converthtml) $text = wordwrap($text, 75, "\n ", true); // MA-MODIFIED 
...
{
    return is_user_on($session, $userid, MDL_F2F_STATUS_WAITLISTED);
}

/**
 * Determine if a user is awaiting manager approval of a session.
 *
 * @param object $session A session object
 * @param int $userid The user ID
 * @return bool True if the user is awaiting approval, false otherwise.
 */
function facetoface_is_user_awaiting_approval($session, $userid = null)
{
    return is_user_on($session, $userid, MDL_F2F_STATUS_REQUESTED);
}

/**
 * Function to check user's signup status
 * 
 * @param object $session A session object
 * @param int $userid The user ID
 * @param int $status facetoface sign up status
 * @return type
 */
function is_user_on($session, $userid = null, $status = null)
{
    global $DB, $USER;
    ...
    $sql = "SELECT 1
          ...
          WHERE su.sessionid = ?
            ...
            AND su.userid = ?
            AND ss.statuscode = ?";
    return $DB->record_exists_sql($sql, array($session->id, $userid, $status));
}
...
function facetoface_get_user_submissions($facetofaceid, $userid, $includecancellations = false)
{
    ...
    $whereclause = "s.facetoface = ? AND su.userid = ? AND ss.superceded != 1 AND ss.statuscode != ?  ";
    $whereparams = array($facetofaceid, $userid, MDL_F2F_STATUS_USER_CANCELLED);
...
function facetoface_print_session($session, $showcapacity, $calendaroutput = false, $return = false, $hidesignup = false)
{
    ...
    if ($session->datetimeknown) {
        ...
        if ($calendaroutput) {
            $html = '';
            $html .= 'Time zone: ' . $session->timezone;
            ...
            foreach ($session->sessiondates as $key => $date) {
                ...
                $start_time = new timezone_class($date->timestart, $user_tz, $session_tz);
                $finish_time = new timezone_class($date->timefinish, $user_tz, $session_tz);
                $timestart  = $start_time->getSessiontime('d M Y, h:i A');
                $timefinish = $finish_time->getSessiontime('d M Y, h:i A');

                //                $timestart_stamp  = ma_facetoface_offset_timestamp($date->timestart,  $server_tz, $session_tz);
                //                $timefinish_stamp = ma_facetoface_offset_timestamp($date->timefinish, $server_tz, $session_tz);
                //                $timestart  = date('d M Y, h:i', $timestart_stamp);
                //                $timefinish = date('d M Y, h:i', $timefinish_stamp);
...
class facetoface_candidate_selector extends user_selector_base
{
    ...
    public function find_users($search)
    {
        global $DB;
        //Get courseid
        $courseid = $DB->get_field_sql('SELECT f.course from mdl_facetoface_sessions fs inner join mdl_facetoface f on fs.facetoface=f.id where fs.id=?', array($this->sessionid));
        ...
        $sql = "
               ...
               ";
        $sql .= " AND u.id in(SELECT ue.userid from mdl_user_enrolments as ue inner join mdl_enrol e on e.id=ue.enrolid and e.status=0 WHERE e.courseid=" . $courseid . " group by ue.userid)";
...
// MA-MODIFIED ===>
// ************** created by mindatlas *******************
// called in theme calendar_renender show_month_detailed()

function mod_facetoface_calendar_attributes($event, $attributes)
{
    global $DB;

    $session = $DB->get_record('facetoface_sessions', array('id' => $event->uuid));
    // $session_dates = $DB->get_record('facetoface_sessions_dates', array('sessionid' => $session->id));

    $title = '';
    // $title.= date('H:i', $session_dates->timestart).' - '.date('H:i', $session_dates->timefinish);

    $attributes['class'] .= ' f2f-event ';
...
/**
 * Return the description for an event
 *
 * @param calendar_event $event
 * @return string
 */
function mod_facetoface_event_description($event)
{
    global $DB, $USER;

    // We need to load uuid from database since moodle's function does not load this when displaying the event
    // in case of mod_facetoface uuid is the session id
    $sessionid = $DB->get_field('event', 'uuid', array('id' => $event->id));

    $facetoface = $DB->get_record('facetoface', array('id' => $event->instance));
    $session = facetoface_get_session($sessionid);

    $signupcount = facetoface_get_num_attendees($session->id);
    $session->placesleft = $session->capacity - $signupcount;
    ...
    $submission_status = get_session_submission_status($sessionid, $USER->id);


    // check session not expired
    $session_upcoming = false;
    foreach ($session->sessiondates as $key => $sessiondate) {
        if ($sessiondate->timestart > time()) {
            $session_upcoming = true;
        }
    }

    if ($eventtype == 'session') { // An event in the calendar not an user's event
        if (
            !empty($submission_status) &&
            $submission_status->statuscode >= MDL_F2F_STATUS_REQUESTED &&
            $submission_status->statuscode < MDL_F2F_STATUS_NO_SHOW
        ) { // There is already a submission
            $description .= get_string('status_' . facetoface_get_status($submission_status->statuscode), 'facetoface');
        } else {

            if ($session_upcoming) { // there is a upcoming session
                $linkurl = new moodle_url('/mod/facetoface/signup.php', array('s' => $session->id));
                if ($facetoface->approvalreqd) {
                    $statustext = get_string('request_approval', 'facetoface');
                } else {
                    // MA: add condition, if no seats available
                    if ($session->placesleft > 0) {
                        $statustext = get_string('signupforthissession', 'facetoface');
                    } else {
                        $statustext = get_string('joinwaitlist', 'facetoface');
                    }
                }
            } else {
    ...
    } else if ($eventtype == 'booking') { // An user's booking event
        $statustext = get_string('status_' . facetoface_get_status($submission_status->statuscode), 'facetoface');
    }

    if (!empty($linkurl)) {
        $description .= '<a class="btn" href="' . $linkurl . '">';
    }

    if (!empty($statustext)) {
        if ($statustext == 'Booked') {
            $description .= ' ';
            $linkurl_cancel = new moodle_url('/mod/facetoface/cancelsignup.php', array('s' => $session->id, 'backtoallsessions' => $facetoface->id));
            $description .= '<button><a class="btn" href="' . $linkurl_cancel . '">';
            $description .= 'Cancel booking';
            $description .= '</a></button>';
        } else $description .= $statustext;
    }
    ...
    if (face_to_face_check_hierarchy() && !is_siteadmin($USER->id) && !empty(face_to_face_get_all_child_users_from_userid($USER->id))) {
        $description .= '<br><br>';
        $linkurl_team = new moodle_url('/mod/facetoface/signup_team.php', array('s' => $session->id));
        $description .= '<button><a class="btn" href="' . $linkurl_team . '">';
        $description .= 'Sign-up for my team';
        $description .= '</a></button>';
    }

    $course_link = new moodle_url('/course/view.php', array('id' => $facetoface->course));
    $description .= '<a class="btn open-course-button" href="' . $course_link . '">Open course</a>';

    $description .= '</div>'; // close .btns

    return $description;
}

function get_session_submission_status($sessionid, $userid)
{
    global $DB;

    return $DB->get_record_sql('SELECT sign_status.*
        FROM {facetoface_signups_status} sign_status
        JOIN {facetoface_signups} signup
        ON signup.id = sign_status.signupid
        WHERE sign_status.superceded = 0
        AND signup.sessionid = ?
        AND signup.userid = ?', array($sessionid, $userid));
}

function get_user_sessions($userid, $limit = null, $extrafields = array(), $include_past = false)
{
    global $DB;

    $fields = array(
        'fsess.id', 'f.name', 'fsess_dates.timestart', 'fsess_dates.timefinish', 'sign_status.statuscode as status'
    );
    ...
    $params = array();
    if (!is_siteadmin($userid)) {
        $condition1 = "INNER JOIN (SELECT e.courseid from mdl_enrol e INNER JOIN mdl_user_enrolments ue on e.id=ue.enrolid and ue.userid=? group by e.courseid) u_enrolled on u_enrolled.courseid=f.course";
        $condition2 = "and fsign.userid = ?";
        $params[] = $userid;
        $params[] = $userid; // We need two userid variables as the parameters for condititions in SQL query
    }
    ...
    $query = "SELECT $fields
        ...
        LEFT JOIN mdl_facetoface_signups fsign
        ON fsign.sessionid = fsess.id " . $condition2 . " 
        LEFT JOIN mdl_facetoface_signups_status sign_status
        ON sign_status.signupid = fsign.id AND sign_status.superceded = 0 AND sign_status.statuscode NOT IN (?, ?) 
        WHERE sign_status.statuscode IS NOT NULL ";
    $params[] = MDL_F2F_STATUS_USER_CANCELLED;
    $params[] = MDL_F2F_STATUS_SESSION_CANCELLED;
    if (!$include_past) {
        $query .= 'AND fsess_dates.timefinish > ?';
        $params[] = time();
    }

    $query .= ' ORDER BY fsess_dates.timestart DESC ';
    ...
    return iterator_to_array($DB->get_recordset_sql($query, $params), false);
}

function get_hierarchy_manager_id_levels($userid, $email = "")
{
    global $DB;
    $arr_managers = array();
    $sql = "SELECT hu1.user_id from mdl_hierarchy_user as hu1 where hu1.node_id in (select hn.parent_node_id from mdl_hierarchy_node as hn inner join mdl_hierarchy_user hu2 on hu2.node_id = hn.id and hu2.user_id=?)";
    ...
    if (!empty($rs)) {
        foreach ($rs as $row) {
            if ($DB->record_exists('user', array('id' => $row->user_id, 'email' => trim($email)))) {
                return $row->user_id;
            } else {
                $arr_managers[] = $row->user_id;
            }
        }
    }
    if (empty($arr_managers)) {
        $sql = 'SELECT hu1.user_id from mdl_hierarchy_user as hu1 where hu1.node_id in (select parent_node_id from mdl_hierarchy_node where id in( select hn.parent_node_id from mdl_hierarchy_node as hn inner join mdl_hierarchy_user hu2 on hu2.node_id = hn.id and hu2.user_id=?))';
        $rs = $DB->get_records_sql($sql, array($userid));
        if (!empty($rs)) {
            foreach ($rs as $row) {
    ...
    } elseif (count($arr_managers) <= 1) return $arr_managers[0];
    else return $arr_managers;
}



/**
 * Return the sessions user signed up 
 * @return array 
 * @param int userid
 * @param bool  if true, include user canceled signup
 * @param array $conditons
 * @param array $params
 * @param string $sort
 */
function ma_get_user_sessions($userid, $cancelled = FALSE, $conditions = [], $params = [], $sort = '')
{
    global $DB, $CFG;

    $user_sessions = array();
    if (empty($sort)) {
        $user_signups = $DB->get_records('facetoface_signups', array('userid' => $userid));
    } else {
        if (!empty($conditions)) {
            $conditions = 'AND ' . implode(' AND ', $conditions);
        }
        $params = array_merge([$userid], $params);
        $user_signups = $DB->get_records_sql("SELECT DISTINCT fs.*
            FROM {facetoface_signups} fs
            LEFT JOIN {facetoface_sessions} fsess
            ON fsess.id = fs.sessionid
            LEFT JOIN {facetoface_sessions_dates} fd
            ON fd.sessionid = fsess.id
            WHERE fs.userid = ?
            $conditions
    ...
    foreach ($user_signups as $signup) {
        $signup_status = $DB->get_record('facetoface_signups_status', array('signupid' => $signup->id, 'superceded' => '0'));
        $session = $DB->get_record('facetoface_sessions', array('id' => $signup->sessionid));
        $facetoface = $DB->get_record('facetoface', array('id' => $session->facetoface));


        $coursemoduleid = $DB->get_field_sql("SELECT cm.id
            FROM {course_modules} cm
            JOIN {modules} m
            ON m.id = cm.module
            WHERE m.name = 'facetoface' AND cm.instance = ?", array($facetoface->id));

        $session->facetoface = $facetoface->id;
        $session->facetoface_name = $facetoface->name;
        $session->course = $facetoface->course;
        $session->url = "$CFG->wwwroot/mod/facetoface/view.php?id={$coursemoduleid}";

        $session->signup = $signup->id;
        $session->signup_statuscode = $signup_status->statuscode;
        $session->signup_status = ma_get_session_signup_status_text($signup_status->statuscode);
        $session->sessiondates = facetoface_get_session_dates($session->id);

        if ($session->signup_statuscode !== '10' || $cancelled == true) {
...
function ma_get_session_signup_status_text($statuscode)
{
    $status_code_text = array(
        MDL_F2F_STATUS_USER_CANCELLED => 'Cancelled',
        MDL_F2F_STATUS_SESSION_CANCELLED => 'Cancelled',
        MDL_F2F_STATUS_DECLINED => 'Declined',
        MDL_F2F_STATUS_REQUESTED => 'Awaiting approval',
        MDL_F2F_STATUS_APPROVED => 'Enrolled',
        MDL_F2F_STATUS_WAITLISTED => 'Wait-listed',
        MDL_F2F_STATUS_BOOKED => 'Enrolled',
        MDL_F2F_STATUS_NO_SHOW => 'Not attended',
        MDL_F2F_STATUS_PARTIALLY_ATTENDED => 'Partially attended',
        MDL_F2F_STATUS_FULLY_ATTENDED => 'Attended'
    );
...
/**
 * Specifically for f2f enrolment status report
 * @param int $statuscode
 * @return string empty string, Not Attended or Attended
 */
function ma_get_enrol_status($statuscode)
{
    $str = '';
    switch ($statuscode) {
        ...
        case MDL_F2F_STATUS_PARTIALLY_ATTENDED:
            $str = 'Partially Attended';
            break;
    }
    return $str;
}

function ma_facetoface_seats_available($session)
{
    return max(0, $session->capacity - facetoface_get_num_attendees($session->id));
}
 ...
 * @param class $session record from the facetoface_sessions table
 */
function ma_facetoface_has_session_finished($session)
{
 ...
 * Only return session data if ma_get_enrol_status() returns a status
 * @param int $faceid facetofaceid
 * @param int $userid userid
 * @return array session data
 */
function ma_get_user_session_data($faceid, $userid)
{

    //error_log($userid . ' ' . var_export(facetoface_get_user_submissions($faceid, $userid, true), true));
    $data = array();
    if ($submissions = facetoface_get_user_submissions($faceid, $userid, true)) {
        //error_log('$submissions ' . var_export($submissions, true));

        foreach ($submissions as $submission) {
            $status = ma_get_enrol_status($submission->statuscode);
            ...
            $data[$sessionid]['enrol_status'] = $status;
            if ($customdata = facetoface_get_customfielddata($submission->sessionid)) {
                foreach ($customdata as $key => $val) {
                    $data[$sessionid][$key] = $val->data;
                }
            }
        }
    }
 ...
 * @param int $userid
 * @param array $fields
 * @return array object of profile data
 */
function ma_get_user_profile_data($userid, $fields = array())
{
    global $DB;
    list($sql_condition, $params) = $DB->get_in_or_equal($fields);
    $params[] = $userid;
    $sql = <<< EOT
SELECT 
    uid.id, 
...
EOT;
    ...
    foreach ($records as $id => $record) {
        $data->{$record->shortname} = $record->data;
    }
    return $data;
}
...
/**
 * Specifically for f2f enrolment status report
 * set profile fields array values
 * @param &array associative array to be set
 */
function ma_set_user_profile_fields(&$fields)
{
    global $DB;
    list($sql_condition, $params) = $DB->get_in_or_equal(array_keys((array)$fields));
    $sql = <<< EOT
SELECT
  id,
  shortname,
...
FROM
  {user_info_field}
WHERE 
  shortname $sql_condition
EOT;

    $records = $DB->get_records_sql($sql, $params);
    foreach ($records as $id => $record) {
        if (!isset($fields[$record->shortname])) {
...
/**
 * Specifically for f2f enrolment status report
 * set user session fields array values
 * @param &array associative array to be set
 */
function ma_set_user_session_fields(&$fields)
{
    global $DB;
    list($sql_condition, $params) = $DB->get_in_or_equal(array_keys((array)$fields));
    $sql = <<< EOT
...
/**
 * custom function to get the details of the facetoface sessions
 * along with the user status for the session
 * @param int $uid, the userid
 * @param int $sessionid, the facetoface session id
 * @return array $return containing session date, location, facilitator, attend 
 */
function get_f2f_detail_user($uid, $sessionid, $courseid)
{
    //require_once('lib.php');
    global $DB;

    $user_sessions = ma_get_user_sessions($uid);
    foreach ($user_sessions as $user_session) {
    }

    $return = array();

    $sql = "SELECT fss.statuscode as user_session_status, fss.note as user_session_note FROM mdl_facetoface_signups_status fss INNER JOIN mdl_facetoface_signups fs ON fs.id = fss.signupid WHERE fs.sessionid = {$sessionid} AND fs.userid={$uid} AND fss.superceded != 1";

    $result = $DB->get_record_sql($sql);
    // echo '<pre>'.print_r($result,1).'</pre>';
    $session_date_times = $DB->get_record_sql("SELECT timestart, timefinish FROM mdl_facetoface_sessions_dates WHERE sessionid = ?", array($sessionid));
    // echo '<pre>'.print_r($session_date_times,1).'</pre>';


    $return['session_date'] = sprintf("%s - %s", date("d/m/Y h:iA ", $session_date_times->timestart), date("d/m/Y h:iA", $session_date_times->timefinish));
    $return['location']     = ma_facetoface_get_session_customfield_data($sessionid, 'Location');
    //on dev room is the shortname for fetching the facilitator value
    $return['facilitator']  = ma_facetoface_get_session_customfield_data($sessionid, 'room');
    $current_time = time();




    if (($session_date_times->timefinish < $current_time)) {
        $return['attend']       = ((isset($result->user_session_status)) and ($result->user_session_status == MDL_F2F_STATUS_FULLY_ATTENDED)) ? 'Y' : 'N';
    } elseif (($session_date_times->timefinish > $current_time) and ($session_date_times->timestart < $current_time)) {
        $return['attend']       = ((isset($result->user_session_status)) and
            (($result->user_session_status == MDL_F2F_STATUS_FULLY_ATTENDED) ||
                ($result->user_session_status == MDL_F2F_STATUS_BOOKED))
        ) ? 'Y' : 'N';
    } elseif (($session_date_times->timestart > $current_time)) {
        $return['attend']       = ((isset($result->user_session_status)) and  ($result->user_session_status == MDL_F2F_STATUS_BOOKED)) ? 'Y' : 'N';
    } else {
        $return['attend']       = '';
    }
...
function facetoface_get_previous_sequence($facetoface, $session, $user)
{
    $result = get_config('facetoface', "sequence-{$facetoface->id}-{$session->id}-{$user->id}");
...
function facetoface_save_sequence($facetoface, $session, $user, $sequence)
{
    return set_config("sequence-{$facetoface->id}-{$session->id}-{$user->id}", $sequence, 'facetoface');
}

function dffh_is_block_formbuilder_installed()
{
    global $DB;

    $formbuilder_block = $DB->get_record('block', array('name' => 'mindatlas_formbuilder'));
    if ($formbuilder_block != false) {
        return true;
    }
    return false;
}
function dffh_list_formbuilder_forms()
{
    global $DB;
...
//Todo: implementation
/**
 * The function generate formbuilder user data if it does not exist
 */
function dffh_generate_formbuider_userdata($f2fsession, $userid)
{
    global $DB;
    if (!$session = facetoface_get_session($f2fsession)) {
    ...
    if ($userid == -1) return;
    $answers = $DB->get_records('formbuilder_form_info_data', array('userid' => $userid, 'f2fsessionid' => $f2fsession));
    ...
    $formfields = $DB->get_records_sql('SELECT * FROM {formbuilder_form_info_field} WHERE formid = ? AND id NOT IN ('.$fields.')', array('formid' => $formid));
    ...
    foreach ($formfields as $field) {
            ...
            $fieldobj->userid = $userid;
            $fieldobj->fieldid = $field->id;
            $fieldobj->f2fsessionid = $f2fsession;
            $fieldobj->formid = $formid;
            $DB->insert_record('formbuilder_form_info_data', $fieldobj);
    }
}
/**
 * The function return formfield data which contains formfield and its selected value
 */
function dffh_view_formbuilder_user($f2fsession, $userid, $formid)
{
    global $DB;
    $sql = "SELECT FF.*, FD.data
            FROM mdl_formbuilder_form_info_field FF
            LEFT JOIN mdl_formbuilder_form_info_data FD
            ON FF.id = FD.fieldid
            WHERE FD.f2fsessionid = ? AND FD.userid = ? AND FF.formid = ? ";
    $formfields = $DB->get_records_sql($sql, array($f2fsession, $userid, $formid));
    if(empty($formfields)){
        $sql = "SELECT * FROM mdl_formbuilder_form_info_field FF
        WHERE formid = ? ORDER BY sortorder ASC";
...
define('DROPDOWN', 'select');
define('RADIO', 'radio');
/**
 * The function return HTLM string 
 */
function dffh_render_formfield_hidden($name, $value)
{
    $html = "<input type=\"hidden\" name=\"$name\" value=\"$value\">";
    return $html;
}
function dffh_render_formfield($formfield,$formid=0)
{
    
   
    if ($formfield->datatype == TEXT_INPUT) return dffh_render_formfield_text($formfield,$formid);
    if ($formfield->datatype == TEXT_AREA) return dffh_render_formfield_textarea($formfield,$formid);
    if ($formfield->datatype == CHECKBOX) return dffh_render_formfield_checkbox($formfield,$formid);
    if ($formfield->datatype == 'select')  return dffh_render_formfield_dropdown($formfield,$formid);
    if ($formfield->datatype == RADIO) return dffh_render_formfield_radio($formfield,$formid);
    return "";
}
function dffh_render_formfield_text($formfield,$formid=0)
{
    global $DB, $USER;
    if($formid == 0){
    ...
    if(!$formbuilderuserid){
        $formbuilderuserid = $USER->id;
    }
    $formid =  $facetoface->formbuilder;
    $elementid = "text" . $formfield->id;
    $options = explode(",", $formfield->defaultdata);
    $dependency = '';
    if($formfield->param1 != 0 && !empty($formfield->param1)){
        $dependency = 'data-dependency="'.$formfield->param1.'"';
        $fieldid = strtok($formfield->param1, '-');
        $optionchosen = substr($formfield->param1, strpos($formfield->param1, "-") + 1);   
        $sql = "SELECT FF.id, FD.data 
        FROM mdl_formbuilder_form_info_field FF
        LEFT JOIN mdl_formbuilder_form_info_data FD
        ON FF.id = FD.fieldid
        WHERE FD.f2fsessionid = ? AND FD.userid = ? AND FF.formid = ? AND FF.id = ? AND FD.data = ? ";
        $answer = $DB->get_record_sql($sql, array($s, $formbuilderuserid, $formid,$fieldid, $optionchosen));
    }
    if($answer || $formfield->param1 == 0){
        $display = '';
    }else{
        $display = 'd-none';
    }
    ...
    $html = "<div data-id='".$formfield->id."' ".$dependency." class=\"form-group ".$display."\">";
    $html .= "<div><h4 >";
    $html .= "$formfield->name</h4></div>";
    $html .= "<input class='' id=\"$elementid\" type=\"text\" name=\"$formfield->shortname\" value=\"$formfield->data\"/> </div>";

    return $html;
}
function dffh_render_formfield_textarea($formfield,$formid=0)
{
    global $DB,$USER;
    ...
    if($formfield->param1 != 0 && !empty($formfield->param1)){
        ...
        LEFT JOIN mdl_formbuilder_form_info_data FD
        ON FF.id = FD.fieldid
        WHERE FD.f2fsessionid = ? AND FD.userid = ? AND FD.formid = ? AND FF.id = ? AND FD.data = ? ";
        $answer = $DB->get_record_sql($sql, array($s, $formbuilderuserid, $formid,$fieldid, $optionchosen));
    }
    if($answer || $formfield->param1 == 0){
        $display = '';
    ...
    $html .= "<div><h4 >";
    $html .= "$formfield->name</h4></div>";
    $html .= "<textarea class='' id=\"$elementid\"  name=\"$formfield->shortname\" value=\"$formfield->data\" rows=\"4\" cols=\"50\">$formfield->data </textarea> </div>";

    return $html;
}
function dffh_render_formfield_checkbox($formfield,$formid=0)
{   
    global $DB, $USER;
    ...
    $html = "<div data-id='".$formfield->id."'".$dependency." class=\"form-group ".$display."\">";
    $html .= "<div><h4  for=\"$elementid\" >";
    $html .= "$formfield->name</h4></div>";
    $options = explode(',',$formfield->defaultdata);
    if(strpos($formfield->data, ',') !== false){
        foreach($options as $option){
                if(strpos($formfield->data, $option) !== false){
                    $html .= "<div class='form-check'><input class='form-check-input' type=\"checkbox\" name=\"$formfield->shortname[]\" value='".$option."' checked/>";
                $html .= '<label class="col-form-label" for="'.$formfield->shortname.'">'.$option.'</label></div>';
                }else{
                    $html .= "<div class='form-check'><input class='form-check-input' type=\"checkbox\" name=\"$formfield->shortname[]\" value='".$option."' />";
                    $html .= '<label class="col-form-label" for="'.$formfield->shortname.'">'.$option.'</label></div>';
                }
        }
...
function dffh_render_formfield_radio($formfield,$formid=0)
{
    global $DB, $USER;
    if($formid == 0){
        $formbuilderuserid = optional_param('attendeeid', -1, PARAM_INT); // userid for taking the coaching data survey
    ...
    foreach($options as $option){
        if ($formfield->data == $option){
            $html .= "<div class='form-check'><input class='form-check-input' type=\"radio\" name=\"$formfield->shortname\" value='".$option."' checked/>";
            $html .= '<label class="col-form-label" for="'.$formfield->shortname.'">'.$option.'</label></div>';
        }else{
            $html .= "<div class='form-check'><input class='form-check-input' type=\"radio\" name=\"$formfield->shortname\" value='".$option."' />";
            $html .= '<label class="col-form-label" for="'.$formfield->shortname.'">'.$option.'</label></div>';
        }
    }
    $html .= "</div>";
...
function dffh_render_formfield_dropdown($formfield,$formid=0)
{
    global $DB, $USER;
    ...
    $html = "<div data-id='".$formfield->id."' ".$dependency." class=\"form-group ".$display."\">";
    $html .= "<h4 for=\"$elementid\" name=\"$formfield->shortname\" value=\"$formfield->name\">";
    $html .= "$formfield->name</h4>";
    $html .= "<select class='form-control' name=\"$formfield->shortname\">";
    $html .= "<option value=\"\">Choose...";
    $html .= "</option>";
    foreach ($options as $option) {
        if ($option == $formfield->data)
...
function dffh_handle_formfield_data_submit($data)
{
    // var_dump($data);
    // die;
    global $DB;
    ...
    foreach ($data as $item => $key) {
        if($item != 'attendeeid' && $item != 'formid' && $item != 's'){
            $stringdata = '';
            ...
            $fieldshortname = $DB->get_record('formbuilder_form_info_field', array('shortname' => $item, 'formid' => $data['formid']));
            if (intval($fieldshortname->visible) == 1) {
                ...
                if($formdataobj){
                    ...
                    $formdataobj->formid = $data['formid'];
                    if(is_array($key)){
                        $formdataobj->data = $stringdata;
                    }else{
                        $formdataobj->data = $key;
                    }
                    ...
                    $fieldobj = new stdClass();
                    $fieldobj->userid = $userid;
                    $fieldobj->fieldid = $fieldshortname->id;
                    $fieldobj->f2fsessionid = $sessionid;
                    $fieldobj->formid = $data['formid'];
                    if(is_array($key)){
                        $fieldobj->data = $stringdata;
                    }else{
                        $fieldobj->data = $key;
                    }
    ...
    $url_back = new moodle_url('/mod/facetoface/attendees.php', array('s' => $sessionid));
    $msg = 'Form was succesfully submitted!';
    redirect($url_back,  \core\notification::success($msg));
