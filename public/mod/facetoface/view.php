...
// MA-MODIFIED ===>
if (isset($_POST['facetoface']) && isset($_POST['userid']) && isset($_POST['interested']) ) {
    if(ma_update_interest($_POST['facetoface'], $_POST['userid'], $_POST['interested'])) {
        redirect($CFG->wwwroot.'/mod/facetoface/view.php?f='.$_POST['facetoface'],
            'An administrator will be notified of your expression of interest.', null, \core\output\notification::NOTIFY_SUCCESS);
    }
}
// <=== MA-MODIFIED 
...
// <=== MA-MODIFIED 
if (has_capability('mod/facetoface:viewattendees', $context) || !empty($team_members)) {
...
function print_session_list($courseid, $facetofaceid, $location) {
    ...
    $viewattendees = has_capability('mod/facetoface:viewattendees', $context) || !empty($team_members);
    ...
    if (empty($upcomingarray) && empty($upcomingtbdarray)) {
        ...
        echo ma_html_interest_btn($facetofaceid, $USER->id); // MA-MODIFIED
    } else {
    ...
    if ($editsessions) {
        $addsessionlink = html_writer::link(
            new moodle_url('sessions.php', array('f' => $facetofaceid,'class'=>'btn')),
