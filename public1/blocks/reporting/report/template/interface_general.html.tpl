  ...
  <h2 class="headingblock header">{php} get_string('general_reports', 'block_reporting') {/php}</h2>
  ...
  <form action="general.php" method="get" name="search">
	...
	<table class="w-100">
	  <tr>
		...
		<td>
			{* @25/07/2018 *}
		  <select name="course[]" class="chzn-select form-control" multiple>
			<option value=""></option>
	  ...
	  <tr>
		<td class="pr-5 color-brand-1"><strong>Completion Date</strong></td>
		<td>
			{* @23/07/2018 new enhancement *}
	...
	{foreach from=$general_filters_array item=general_filters}
		<tr>
			...
			<td>
				...
				{else}
					{* @25/07/2018 enhancement *}
					<select name="{$general_filters->filtername}[]" id="{$general_filters->filtername}" class="chzn-select form-control" multiple>
						<option value=""></option>
	...
	<tr>
		<td class="pr-5 color-brand-1"><strong>Suspended users</strong></td>
		<td>
  ...
  {literal}
      ...
      // disable completion date if Not Completed is selected
      $('#completionstatus').change(function() {
        toggleCompletionDates();
      });
      
      // Check if Not Completed is selected when page is loaded
      toggleCompletionDates();
