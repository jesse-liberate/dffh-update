...
$prop_name = 'completion_full_cron_running';

set_config($prop_name, 1, $plugin_name);
...
$where=" WHERE u.deleted=0 and u.suspended=0 and cm.completion<>0 ";
...
if ($STH) {
	...
	if(count($rows)>1){
		...
		// if(($value["completion"] == '')||($value["completion"]===null)||($value["completion"] == 0)) continue;
		// Nn need to insert Guest user (id = 1), deleted user and suspended users
		if ($value['userid'] != 1 && !$value['deleted']) {
...
set_config($prop_name, 0, $plugin_name);