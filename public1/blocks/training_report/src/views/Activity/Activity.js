...
import SingleCourseSelector from '../../components/SingleCourseSelector';
...
export default class Activity extends React.Component{
  ...
  componentDidMount(){
    ...
    axios.post(`${M.cfg.wwwroot}/blocks/training_report/api/report.php`, {
      ...
      this.setState({
        courses: Object.values(res.data.data.courses),
  ...
  exportCSV = () => {
    //send current selected data to BE, get data back
      ...
      today = dd + '-' + mm + '-' + yyyy;
      var filename = 'sgl_element_summary-' + today + '.csv';
  ...
  exportPDF = () => {
    ...
    payload: {
      ...
      this.exportPDFFile(res.data.data.headers,res.data.data.data,"SGL Element Summary","SGL Element Summary.pdf");
      this.setState({isLoading: false})
    })
    .catch(function (error) {
    // handle error
  ...
  render() {
    const {courses,completionFilters, suspendedFilters, hierarchyNodes,
      addtionalFilters,reportCode,reportHeaders,reportBody,isLoading,
      isFirst,sortStatus,recordperpage,total_page,pagenum,order_by,
      exportTypeFilters,display,submittedDisplay,isError,errorMsg} = this.state
    return (
        <div className="row">
          <div className="filter-zone col-lg-3">
              <div className="w-100 tw-p-3">
                  <FilterPanel>
                    <SingleCourseSelector courses={courses} onValueChanged={value=>this.saveData('selectedCourseId', value.value)} />  
                    {/* <CompletionSelector completion={this.state.completionFilters} onValueChanged={this.onCompletionSelected} /> */}
                    {/* <CompletionSelector completion={this.state.completionFilters} onValueChanged={this.saveDropdownData('completion')} /> */}
