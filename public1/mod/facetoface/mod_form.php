...
class mod_facetoface_mod_form extends moodleform_mod {
    ...
    public function definition() {
        ...
        $available_forms = dffh_list_formbuilder_forms();
        $formoptions = array();
        foreach ($available_forms as $form) {
            $formoptions[$form->id] = format_string($form->name);
        }
        $mform->addElement('select', 'formbuilder', 'Available Forms', $formoptions);
        $mform->addHelpButton('formbuilder', 'facetoface_data_help', 'facetoface');
        ...
        $mform->setDefault('approvalreqd', false);   // MA-MODIFIED
        $mform->addHelpButton('approvalreqd', 'approvalreqd', 'facetoface');

        // MA-MODIFIED ===>
        $mform->addElement('checkbox', 'streamlinedaccess', get_string('streamlinedaccess', 'facetoface'));
        $mform->setDefault('streamlinedaccess', true);  // MA-MODIFIED
        $mform->addHelpButton('streamlinedaccess', 'streamlinedaccess', 'facetoface');
        // <=== MA-MODIFIED 
        ...
        //MindAtlas - apply for course level only
        ...
        // $usercalentry = $mform->addElement('advcheckbox', 'usercalentry', get_string('usercalentry', 'facetoface'));
        // $mform->setDefault('usercalentry', false);
        // $usercalentry->freeze();
        ...
        $mform->addElement('editor', 'requestmessage', get_string('email:message', 'facetoface'), 'wrap="virtual" rows="15" cols="70"');
        $mform->setDefault('requestmessage', array(
            'text' => get_string('setting:defaultrequestmessagedefault', 'facetoface'),
            'format'=> FORMAT_HTML
        )
    );
        ...
        $mform->addElement('editor', 'confirmationmessage', get_string('email:message', 'facetoface'), 'wrap="virtual" rows="15" cols="70"');
        $mform->setDefault('confirmationmessage', array(
            'text' => get_string('setting:defaultconfirmationmessagedefault', 'facetoface'),
            'format'=> FORMAT_HTML
        )
    );
        ...
        // Booking change message
        $mform->addElement('header', 'booking_change', get_string('booking_changemessage', 'facetoface'));
        $mform->addHelpButton('booking_change', 'booking_changemessage', 'facetoface');

        $mform->addElement('text', 'booking_changesubject', get_string('email:subject', 'facetoface'), array('size' => '55'));
        $mform->setType('booking_changesubject', PARAM_TEXT);
        $mform->setDefault('booking_changesubject', get_string('setting:defaultbooking_changesubjectdefault', 'facetoface'));

        $mform->addElement('editor', 'booking_changemessage', get_string('email:message', 'facetoface'), 'wrap="virtual" rows="15" cols="70"');
        $mform->setDefault('booking_changemessage', array(
            'text' => get_string('setting:defaultbooking_changemessagedefault', 'facetoface'),
            'format'=> FORMAT_HTML
        )
    );
        $mform->addElement('advcheckbox', 'booking_change_notification', get_string('booking_change_notification', 'facetoface'));
        $mform->setDefault('booking_change_notification', 0); // Set default to disabled
        $mform->addHelpButton('booking_change_notification', 'booking_change_notification', 'facetoface');
        // $mform->setType('booking_change_notification', PARAM_BOOL);
        ...
        $mform->addElement('editor', 'remindermessage', get_string('email:message', 'facetoface'), 'wrap="virtual" rows="15" cols="70"');
        $mform->setDefault('remindermessage', array(
            'text' => get_string('setting:defaultremindermessagedefault', 'facetoface'),
            'format'=> FORMAT_HTML
        )
    );
        ...
        $mform->setDefault('reminderperiod', 7);    // MA-MODIFIED
        ...
        $mform->addElement('editor', 'waitlistedmessage', get_string('email:message', 'facetoface'), 'wrap="virtual" rows="15" cols="70"');
        $mform->setDefault('waitlistedmessage', get_string('setting:defaultwaitlistedmessagedefault', 'facetoface'));
        $mform->setDefault('waitlistedmessage', array(
            'text' => get_string('setting:defaultwaitlistedmessagedefault', 'facetoface'),
            'format'=> FORMAT_HTML
        )
    );
        ...
        $mform->addElement('editor', 'cancellationmessage', get_string('email:message', 'facetoface'), 'wrap="virtual" rows="15" cols="70"');
        $mform->setDefault('cancellationmessage', array(
            'text' => get_string('setting:defaultcancellationmessagedefault', 'facetoface'),
            'format'=> FORMAT_HTML
        )
    );
    ...
    public function set_data($data) {
        if (!empty($data->requestmessage)) {
            $data->requestmessage = ['text' => $data->requestmessage];
        } else {
            $data->requestmessage = get_string('setting:defaultrequestmessagedefault', 'facetoface');
        }
        
        if (!empty($data->confirmationmessage)) {
            $data->confirmationmessage = ['text' => $data->confirmationmessage];
        } else {
            $data->confirmationmessage = get_string('setting:defaultconfirmationmessagedefault', 'facetoface');
        }
        
        if (!empty($data->booking_changemessage)) {
            $data->booking_changemessage = ['text' => $data->booking_changemessage];
        } else {
            $data->booking_changemessage = get_string('setting:defaultbooking_changemessagedefault', 'facetoface');
        }
        
        if (!empty($data->remindermessage)) {
            $data->remindermessage = ['text' => $data->remindermessage];
        } else {
            $data->remindermessage = get_string('setting:defaultremindermessagedefault', 'facetoface');
        }
        
        if (!empty($data->waitlistedmessage)) {
            $data->waitlistedmessage = ['text' => $data->waitlistedmessage];
        } else {
            $data->waitlistedmessage = get_string('setting:defaultwaitlistedmessagedefault', 'facetoface');
        }
        
        if (!empty($data->cancellationmessage)) {
            $data->cancellationmessage = ['text' => $data->cancellationmessage];
        } else {
            $data->cancellationmessage = get_string('setting:defaultcancellationmessagedefault', 'facetoface');
        }
