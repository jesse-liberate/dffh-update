...
class mod_facetoface_session_form extends moodleform {
    ...
    public function definition() {
        ...
        if ($mform->elementExists('custom_location')) {
            ...
            $mform->addRule('custom_location', null, 'required', null, 'client');
        }
        ...
        $formarray  = array();
        $mform->addElement('textarea', 'additional_details', get_string('additional_details', 'facetoface'),array('cols' => 60, 'rows' => 3));
        if ($mform->elementExists('additional_details')) {
            $mform->addHelpButton('additional_details', 'additional_details', 'facetoface');
            $mform->addRule('additional_details', null, 'required', null, 'client');
        }
        ...
        $mform->addRule('details_editor', null, 'required', null, 'client');
        // $mform->addElement('filemanager', 'sessionimage', 'Session Image', null, array('subdirs' => 0, 'maxbytes' => 
        // 0, 'maxfiles' => EDITOR_UNLIMITED_FILES,'accepted_types' => array('image')));
        // $mform->addRule('sessionimage', null, 'required', null, 'client');
        $formarray[] = $mform->createElement('selectyesno', 'datetimeknown', get_string('sessiondatetimeknown', 'facetoface'));
        // MA-MODIFIED ===>
        $select_timezone = $mform->createElement('select', 'sessiontimezone', 'Timezone', 
            array(  'Australia/Adelaide'=>'Australia/Adelaide', 
        ...
        $select_timezone->setSelected('Australia/Sydney'); // key
        $formarray[] = $select_timezone;
        // <=== MA-MODIFIED 
        ...
        $mform->setDefault('datetimeknown', true);  // MA-MODIFIED
        ...
        $date_time_selector_options = array(
          'timezone' => $this->_customdata['server_tz']
        );
        // date_time_selector include timezone to display human readable time
        $repeatarray[] = &$mform->createElement(
          'date_time_selector', 'timestart', 
          get_string('timestart', 'facetoface'), $date_time_selector_options
        );
        $repeatarray[] = &$mform->createElement(
          'date_time_selector', 'timefinish', 
          get_string('timefinish', 'facetoface'), $date_time_selector_options
        );
        ...
        $mform->addGroupRule('timestart[0]', null, 'required', null, 'client');
        $mform->addGroupRule('timefinish[0]', null, 'required', null, 'client');
        ...
        $mform->setDefault('allowoverbook', true);  // MA-MODIFIED
        ...
        if($this->_customdata['s']){
            $mform->addElement('html', '<div class="fitem"><div class="fitemtitle"><label>Notes</label></div>
            <div class="felement">'.get_string('session_datechanges_users','facetoface').'</div>
            </div>');
        }
    ...
    public function validation($data, $files) {
        ...
        if($data['timestart'][0] == $data['timefinish'][0]){
            $errors['datetimeknown'] = 'Please choose date';
        }
        ...
        for ($i = 0; $i < $dates; $i++) {
            ...
            $removecheckbox = empty($data["datedelete"]) ? array() : $data["datedelete"];
            if ($starttime == $endtime && !isset($removecheckbox[$i])) {
                $errstr = get_string('error:sessiondate', 'facetoface');
                $errors['timestart'][$i] = $errstr;
                $errors['timefinish'][$i] = $errstr;
                unset($errstr);
            }
