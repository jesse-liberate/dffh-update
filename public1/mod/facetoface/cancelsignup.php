        ...
        if ($session->datetimeknown) {
            ...
            if (empty($error)) {
                if ($session->datetimeknown && $facetoface->cancellationinstrmngr) {
                   ...
                   // $message .= html_writer::empty_tag('br') . html_writer::empty_tag('br') . get_string('cancellationsent', 'facetoface');   // MA-MODIFIED
                }
            } else {
            ...
            $subject = get_string('sessioncancel_subject','facetoface',(object)['facetofacename'=>$facetoface->name,'coursename'=>$course->shortname]);
            ...
            $htmlcontent = get_string('sessioncancel_waitlist', 'facetoface', (object)[
                'facetofacename' => $facetoface->name,
                ...
                'venue' => $venue,
                'room' => $room,
                'support_contact_name' => fullname($support_user),
                'support_contact_email' => $support_user->email
            ]);
            add_to_mynotification($USER->id,$subject,$htmlcontent);
            $manager_id = get_hierarchy_manager_id($USER->id);
            if(is_array($manager_id)){
                foreach ($manager_id as $mgr_id) {
                    add_to_mynotification($USER->id,$subject,$htmlcontent,$mgr_id);
                }
            }else add_to_mynotification($USER->id,$subject,$htmlcontent,$manager_id);
            // <=== MA-MODIFIED 
        }
