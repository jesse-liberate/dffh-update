...
if (dffh_is_block_formbuilder_installed()) {
    $settings->add(new admin_setting_heading(
        'facetoface_data_form',
        'Data form',
        ''
    ));
    // Load sessions
    ...
    foreach ($available_forms as $form) {
        $options[$form->id] = format_string($form->name);
    }
    $settings->add(new admin_setting_configselect(
        'facetoface_data_form',
        'Available Forms',
        'Select a form for collecting user data',
        '',
        $options
    ));
}
...
// var_dump(get_config(null, 'facetoface_data_form'));