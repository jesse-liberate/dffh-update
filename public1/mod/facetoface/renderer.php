...
class mod_facetoface_renderer extends plugin_renderer_base {
    ...
    public function print_session_list_table($customfields, $sessions, $viewattendees, $editsessions) {      
        ...
        // $tableheader[] = get_string('timezone', 'facetoface');; // column for timezone // MA-MODIFIED
        if ($viewattendees) {
...
//                error_log('$server_tz ' . var_export($server_tz, true));
//                error_log('$session_tz ' . var_export($session_tz, true));
//                error_log('$user_tz ' . var_export($user_tz, true));
                ...
                foreach ($session->sessiondates as $date) {
                    ...
                    // Date column
                    $allsessiondates .= $start_time->getUsertime($dateformat);                  
                    if (!empty($allsessiontimes)) {
                        $allsessiontimes .= html_writer::empty_tag('br');
                    }

                    // Time column
                    $user_timestart  = $start_time->getUsertime($timeformat);
                    $user_timefinish = $finish_time->getUsertime($timeformat);
                    $allsessiontimes .= $user_timestart . ' - ' . $user_timefinish;

                    // Timezone column
                    $session_timestart  = $start_time->getSessiontime($timeformat);
                    $session_timefinish = $finish_time->getSessiontime($timeformat);
                    $timezone .= "$session_tz";
                    $timezone .= html_writer::empty_tag('br');
                    $timezone .= "$session_timestart &ndash; $session_timefinish";
                    $timezone .= html_writer::empty_tag('br');                   
            ...
            if ($session->datetimeknown && facetoface_has_session_started($session, $timenow) && facetoface_is_session_in_progress($session, $timenow)) {
                ...
                $sessionstarted = true;
            } else if ($bookedsession && $session->id == $bookedsession->sessionid && isset($bookedsession->statuscode)&&!is_null($bookedsession->statuscode)) {    // MA-MODIFIED
            ...
            if ($isbookedsession) {
                ...
                if ($session->allowcancellations) {
                    $options .= html_writer::link('cancelsignup.php?s=' . $session->id . '&backtoallsessions=' . $session->facetoface, get_string('cancelbooking', 'facetoface'), array('title' => get_string('cancelbooking', 'facetoface'),'class'=>'btn'));
                }
            } else if (!$sessionstarted and !$bookedsession) {  // MA-MODIFIED
                if ($signupcount >= $session->capacity) { // fully booked
                    $options .= html_writer::link('signup.php?s='.$session->id.'&backtoallsessions='.$session->facetoface, get_string('joinwaitlist', 'facetoface'));
                }else{
                    $options .= html_writer::link('signup.php?s='.$session->id.'&backtoallsessions='.$session->facetoface, get_string('signup', 'facetoface'),array('class'=>'btn'));
                }
